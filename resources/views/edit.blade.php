            ...
            <div class="p-6 bg-white">
                ...
                <form action="{{ route('pasien.update', $pasien->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_pasien" id="nama_pasien" value="{{ $pasien->nama_pasien }}" class="bg-gray-100 border border-gray-500 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="jenis_kelamin_pasien" id="jenis_kelamin_pasien" class="bg-gray-100 border border-gray-500 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                            <option value="L" {{ $pasien->jenis_kelamin_pasien == 'L' ? 'selected' : '' }}>Laki-laki</option>
                            <option value="P" {{ $pasien->jenis_kelamin_pasien == 'P' ? 'selected' : '' }}>Perempuan</option>
                        </select>
                    </div>

                    <div class="mb-4">
                        <label for="tgl_lahir_pasien" class="block text-gray-700 font-semibold mb-1">Tanggal Lahir:</label>
                        <input type="date" name="tgl_lahir_pasien" id="tgl_lahir_pasien" value="{{ $pasien->tgl_lahir_pasien }}" class="bg-gray-100 border border-gray-500 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required>
                    </div>

                    <div class="mb-4">
                        <label for="alamat_pasien" class="block text-gray-700 font-semibold mb-1">Alamat Pasien:</label>
                        <textarea name="alamat_pasien" id="alamat_pasien" class="bg-gray-100 border border-gray-500 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required>{{ $pasien->alamat_pasien }}</textarea>
                    </div>
